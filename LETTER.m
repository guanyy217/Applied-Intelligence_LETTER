function [Outputs,Pre_Labels]=LETTER(train_data,train_target,test_data,test_target,ratio,svm,lambda1,lambda2,MaxIter)
    ...
    for i=1:num_class
        ...
        if(k_i==0)
            [POS_C,NEG_C] = LETTER_Update_InstanceCenters([], train_data, min(50,num_train), lambda1,lambda2,MaxIter,'EmptyAction','singleton','OnlinePhase','off','Display','off');                
        else 
            [POS_C,NEG_C] = LETTER_Update_InstanceCenters(p_data, n_data, k_i, lambda1, lambda2, MaxIter,'EmptyAction','singleton','OnlinePhase','off','Display','off');           
